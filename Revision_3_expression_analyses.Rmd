...
bind_rows(res, .id = "line") |>
  ...
  summarize(l2fc = mean(log2FoldChange), perc = first(Percentage)) |>
...
p53re_res_line_out <- lapply(seq_along(res), function(i) {
  p53re_res_line |>
...
for (i in 1:3) {
  ...
  if (i == 2) {
    print(p + stat_compare_means(comparisons = list(c("0", "1"), c("0", "2"), c("0", "3"), c("0", "4")), method.args = list(alternative = "less")))
  } else {
    print(p + stat_compare_means(comparisons = list(c("0", "1"), c("0", "2"), c("0", "3"), c("0", "4"), c("0", "5")), method.args = list(alternative = "less")))
  }
}
dev.off()
