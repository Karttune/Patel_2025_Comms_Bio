...
# Setting the PATH variable for bedr package to use tabix and bedtools
Sys.setenv(PATH = paste0(Sys.getenv("PATH"), ":", "/data/miniconda2/envs/gradu_3.8/bin"))
...
tabix_mbed <- function(querypath, dbpath = NULL, by = c("read", "call"), extcol = NULL, verbose = TRUE) {
  ...
  if (!is.null(dbpath)) {
    region.tb <- tabix(querypath, dbpath, verbose = verbose)
    colnames(region.tb) <- mbedcnames
  } else {
...
makeMethPlot <- function(methylation, peaks, extension = NULL, win = 50) {
  if (is.null(extension)) {
    ...
    peaks_extended <-
      resize(peaks, fix = "center", width = extension)
    coords <-
      methylation[overlapsAny(methylation, peaks_extended), ]
  }

  cov <- getCoverage(coords, type = "Cov", what = "perBase")
  m <- getCoverage(coords, type = "M", what = "perBase")

  nearest <- nearest(coords, peaks)

  meth_table_final <- as_tibble(granges(coords)) %>%
    mutate(
      cov = cov[, 1],
      meth = m[, 1],
      unmeth = cov - meth,
      center = start(peaks[nearest]),
      dist = start - center
    )

  agg <- aggregate_methylation(meth_table_final, win = win)
...
aggregate_methylation <- function(dat.dist, win = 50) {
  ...
  dat.ag <- dat.dist %>%
    group_by(dist) %>%
    dplyr::summarize(freq = mean(freq, na.rm = T)) %>%
    na.omit()
  roll.range <- seq(
    from = min(dat.ag$dist) + win / 2,
...
#     hist
# }
gpcPeakCaller <- function(bsobj, minwin = 50, a = 0.05, cutoff = NULL, qcutoff = 0.99) {
...
findDMRs <- function(bsobj, onei, twoi, regions = NULL, maxGap = 300, cutoffs = NULL, qcutoffs = 0.9, min_width = 100, min_n = 4, a = 0.05, verbose = T) {
...
getRuns_fast <- function(calls, verbose = F) {
  if (verbose) {
    message("splitting reads by qname and chrom")
  }
...
order_reads <- function(x, offset = 0, bounds = NULL, qorder = NULL) {
  ...
  x <- x %>%
    mutate(
      ymin = bounds$ymin[match(qname, bounds$qname)],
...
mbedByCall_modified <- function(mbed, region = NULL, pad = 2000, verbose = T) {
  if (verbose) cat("parsing data into single call per line\n")
  calls <- NA
  if (nrow(mbed) != 0) {
    if (is.null(region)) {
      out.list <- lapply(seq(dim(mbed)[1]), function(i) {
        ...
        out <- tibble(
          ...
          score = as.numeric(strsplit(read$scores, ",")[[1]]),
          context = strsplit(read$context, ",")[[1]],
          ins_read = read$ins_read,
          ins_read_span = read$ins_read_span
        )
        out
      })
    } else {
...
smoothCalls_modified <- function(calls, what = c("all", "site"), reg = NULL, bandwidth = 40, thr = 0.2, pad = 1000) {
  # "what" is whether I'm only calling on the sites or estimating every nucleotide; default all
  if (length(what) != 1) {
    what <- "all"
  }
  ...
  tibble(
    ...
    ins_read = calls$ins_read[1],
    ins_read_span = calls$ins_read_span[1]
  )
}

getRuns_fast_modified <- function(calls, verbose = F) {
  if (verbose) {
    message("splitting reads by qname and chrom")
  }
  calls <- calls[calls$mcall != -1, ] %>%
  ...
  runs.list <- lapply(calls.list, function(x) {
    rle(x$mcall) %>%
      ...
      mutate(
        ...
        width = end - start,
        ins_read = x$ins_read[1],
        ins_read_span = x$ins_read_span[1]
      ) %>%
      filter(width >= 0) # remove negative widths (in case of dups, etc.)
  })
  runs <- bind_rows(runs.list)
...
getRuns_modified <- function(calls, maxGap = NULL, pad = 0) {
  if (!is.null(maxGap)) {
    pad <- maxGap / 4
    indices <- c(0, cumsum(diff(calls$start) > maxGap)) # based on difference to previous call
    calls$indices <- indices
...
##############################################################################
#### BSseq method for reading modkit bedMethyl input ####
#### Yanked from the developmental branch of the package (commit 7c309d3) ####
##############################################################################
...
read.modkit <- function(files,
  ...
  for (i in seq_along(files)) {
    ...
    if (length(unique(data$V4)) == 2) {
      gr <- GenomicRanges::GRanges(
        seqnames = data[data$V4 == "m", ]$V1,
        ranges = IRanges::IRanges(
          start = data[data$V4 == "m", ]$V2 + 1,
      ...
      gr <- GenomicRanges::GRanges(
        seqnames = data$V1,
        ranges = IRanges::IRanges(start = data$V2 + 1, end = data$V3),
        strand = data$V6
      )
  ...
  overlap_gr <- Reduce(bsseq::subsetByOverlaps, gr_list)
  
  m_u_list <- lapply(gr_list, function(gr) {
    overlap_data <- gr[gr %over% overlap_gr]
    if (!is.null(gr$h)) {
      data.frame(
        m = overlap_data$m, u = overlap_data$u,
  ...
  if (!is.null(h)) {
    ...
    bsseq_obj <- bsseq::BSseq(
      M = as.matrix(m), Cov = as.matrix(u + m),
      # Filtered = as.matrix(filter),
      coef = NULL, se.coef = NULL,
      pos = start(overlap_gr), trans = NULL,
      parameters = NULL, pData = colData, gr = overlap_gr,
